List backlog items for the idea identified by `slug`, sorted oldest-first. Same shape and filter args as `list_backlog`: returns `{id, title, status, created, updated, source?, assignee_session?, external_url?, depends_on?, affects?, labels?}` with `body` dropped by default.
...
- `labels` (string[], optional) — filter to items whose `labels` intersect the given set (any-overlap match). Case-sensitive. Omit or pass `[]` to return all.
